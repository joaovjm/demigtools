          ...
          {dados && dados.length > 0 ? (
            dados.map((item) => (
              <Fragment key={item.receipt_donation_id}>
                <tr onClick={() => handleEditDonation(item)} className="trBody">
                  ...
                  <td className="tableBody">
                    {new Date(item.donation_day_contact).toLocaleDateString(
                      "pt-BR"
                    )}
                  </td>
                  <td className="tableBody">
                    {new Date(item.donation_day_to_receive).toLocaleDateString(
                      "pt-BR"
                    )}
                  </td>
                  <td className="tableBody">
                  ...
                  <td className="tableBody">
                    {item.donation_monthred
                      ? `${new Date(item?.donation_monthref).toLocaleDateString(
                          "pt-BR"
                        )}`
                      : ""}
                  </td>
